...
from . const import LANGUAGE_CHOICE, DEDICATE_HOURS_CHOICE, MEDICAL_QUAL_CHOICES, TIME_PREF_CHOICES, TMV, TMP
...
class TMVolunteerForm(forms.Form):
    ...
    language = forms.MultipleChoiceField(widget=forms.CheckboxSelectMultiple,
                                         choices=list(zip(LANGUAGE_CHOICE, LANGUAGE_CHOICE)))
    time_pref = forms.ChoiceField(widget=forms.Select,
                             choices=list(zip(TIME_PREF_CHOICES, TIME_PREF_CHOICES)), help_text="")
